...
from ..algorithm.pressingtime import presssaver
from ..algorithm.utils import safe_filename, download_file, cleanup_temp_file
...
pressingtime = on_command("pressingtime")
...
async def handle_pressingtime(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        result = await loop.run_in_executor(
            None, presssaver, str(base_path), str(CACHE_DIR)
        )
        
        status = result["status"]
        output_path = result["img"]

        if status == "OK":
            pass
        elif status == "NotMania":
            await pressingtime.send("该回放不是 Mania 模式。")
            return
        elif status == "tooFewKeys":
            await pressingtime.send("有效轨道数量过少，无法分析。")
            return
        else:
            await pressingtime.send("分析失败，请稍后重试。")
            return
