...
async def handle_first(event: MessageEvent, state: T_State):
    ...
    if not event.reply:
        if bid is None:
            await scatter.finish("请回复一条包含 .osr 文件的消息，或使用 b<谱面ID> 指定谱面。")
        else:
            await scatter.finish("请回复一条包含 .osr 文件的消息。")
        # return
...
@scatter.got("user_file", prompt="未提供谱面 ID, 请发送对应的.osu文件。")
...
async def handle_file(state: T_State, user_file: Message = Arg()):
    ...
    if not file_seg:
        await scatter.finish("未找到 .osu 文件，操作已取消。")
